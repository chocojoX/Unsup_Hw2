...
def test_ksubspaces_clustering():
    ...
    plt.title("Evolution of error with dimension at optimal K (with individuals 1-2)")
    plt.ylabel("Error")
    plt.xlabel("dimension of subspaces")
    plt.show()

    to_plot_d_fixed = [perfs[(K, best_params['d'])] for K in Ks]
    plt.plot(Ks, to_plot_d_fixed)
    plt.title("Evolution of error with K at optimal dimension (with individuals 1-2)")
    plt.ylabel("Error")
    plt.xlabel("K")
    plt.show()
